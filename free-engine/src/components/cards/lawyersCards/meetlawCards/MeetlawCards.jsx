...
export const MeetlawCards = observer(() => {
  const lawyersStore = useLawyersStore();
  const [meetlaw, setMeetlaw] = useState(null);
  const [currentPage, setCurrentPage] = useState(1);
  const [totalPages, setTotalPages] = useState(0);
  const itemsPerPage = 6;

  useEffect(() => {
    if (lawyersStore.meetlaw.length > 0) {
      console.log(lawyersStore.meetlaw)
      setMeetlaw(lawyersStore.meetlaw)
      setTotalPages(Math.ceil((lawyersStore.meetlaw.length - 1) / itemsPerPage))
    }
  }, [lawyersStore.meetlaw, currentPage])

  function getMeetlawCards() {
    if (lawyersStore.loadingMeetlaw) {
      return (
    ...
    if (meetlaw) {
      ...
      const actualLawyers = meetlaw.slice(0, -1)
  ...
  return (
    <>
      <Typography gutterBottom variant="body" component="div" marginLeft={"2vh"} sx={{ color: "white", marginTop: "1vh"}}>
        {meetlaw == null ? "Attente de résultats" : lawyersStore.meetlaw[lawyersStore.meetlaw.length - 1]} sur Malt.fr
      </Typography>
